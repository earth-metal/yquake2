...
{
	int nmods = 0, numdirchildren, numpacktype;
	char **dirchildren, findnamepattern[MAX_OSPATH], modname[MAX_QPATH], **modnames;
		...
		for (int i = 0; i < numdirchildren && nmods < MAX_MODS; i++)
		{
			...
			for (int j = 0; j < sizeof(fs_packtypes) / sizeof(fs_packtypes[0]); j++)
			{
				if (strcmp("zip", fs_packtypes[j].suffix) != 0)
				{
					...
					FS_ListFiles(findnamepattern, &numpacktype, 0, 0);
